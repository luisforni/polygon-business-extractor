...
function BusinessCard({ business, selected, onSelect }: BusinessCardProps) {
  return (
    <div
      onClick={onSelect}
      className={clsx(
...
export default function SearchTab({ selectedBusinessId, onBusinessSelect }: SearchTabProps) {
      ...
      <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 gap-3">
        {filtered.map((biz) => (
          <BusinessCard
            key={biz.id}
            business={biz}
            selected={biz.id === selectedBusinessId}
            onSelect={() => onBusinessSelect(biz.id)}
          />
        ))}
